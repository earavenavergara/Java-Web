...
public class Teams extends HttpServlet {
		...
		if (request.getParameter("toDeleteId") != null) {// Viene de index a borrar un equipo
			...
			if (request.getParameter("id") != null) {
				...
				if (Roster.getRoster().get(teamId) == null) {
					// Creo el arraylist de los players de inmediato para evitar problemas al
					// desplegar los nombres de los players
					...
					// establezco el arraylist de los jugadores de este team con id=teamId en el
					// atributo "players" de sesión
					session.setAttribute("players", Roster.getRoster().get(teamId));
				}
			}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("name") == null) { // Viene de index
			...
			// Este ArrayList es especificamente para mostrar el nombre de cada team en los
			// JSP
			ArrayList<String> newNames = (ArrayList<String>) session.getAttribute("names");
